  ...
  return (
    <CyberCard className="p-6">
      ...
      <form onSubmit={handleDemoUpdate} className="space-y-4">
        ...
        <div><label className="mb-1.5 block text-[10px] tracking-[1.5px] text-zinc-400">SLUG</label><input type="text" value={form.slug} onChange={(e) => setForm({ ...form, slug: e.target.value.toLowerCase().replace(/\s+/g, '-') })} className="futuristic-input w-full rounded-xl border border-white/10 bg-zinc-950/60 px-4 py-2.5 text-sm font-mono" required /><div className="mt-1 text-xs text-zinc-500 dark:text-zinc-500 text-zinc-400">URL: /org/{form.slug}</div></div>
